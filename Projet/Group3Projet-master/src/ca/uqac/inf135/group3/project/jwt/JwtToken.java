package ca.uqac.inf135.group3.project.jwt;
...
public class JwtToken {
    ...
    private byte[] computeSignature(byte[] secret) {
        try {
            ...
            byte[] headerAndPayload = String.format("%s.%s", getBase64Header(), getBase64Payload()).getBytes(CHARSET);

            //Return Hash of the header and payload, hashed with secret key
            return HashMacManager.getHashMac(headerAndPayload, secret);
        }
        catch (Exception e) {
